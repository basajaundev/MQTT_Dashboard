    ...
    if (Notification.permission !== "denied") {
        Notification.requestPermission().then((permission) => {
            if (permission === "granted") {
                ...
                const notificationsToggle = document.getElementById('notificationsEnabled');
                if (notificationsToggle) notificationsToggle.checked = true;
                showBrowserNotification("Notificaciones habilitadas", "Recibirás alertas en tiempo real", "info");
            }
        });
    }
}
...
export function showBrowserNotification(title, body, type = 'info', tag = 'general') {
...
export function handleNotificationEvent(data) {
    showBrowserNotification(data.title, data.body, data.type, data.tag);
