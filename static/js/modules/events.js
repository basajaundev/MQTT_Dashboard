...
export async function initEventListeners() {
    document.body.addEventListener('click', async (e) => {
        ...
        // Si es la página de detalles, usar state.deviceDetailData
        ...
        switch(action) {
            ...
            case 'request-device-status': {
                ...
                // También solicitar datos actuales desde el servidor
    ...
    if (elements.taskModal) {
        const scheduleType = elements.taskModal.querySelector('#scheduleType');
        scheduleType.addEventListener('change', () => {
            elements.taskModal.querySelectorAll('.schedule-option').forEach(o => o.style.display = 'none');
            elements.taskModal.querySelector(`#${scheduleType.value}Options`).style.display = 'block';
        });
    }

    if (elements.triggerModal) {
        const triggerActionType = elements.triggerModal.querySelector('#triggerActionType');
        triggerActionType.addEventListener('change', () => {
            const publishOptions = elements.triggerModal.querySelector('#triggerPublishOptions');
            if (publishOptions) {
                publishOptions.style.display = triggerActionType.value === 'publish' ? 'block' : 'none';
            }
        });
    }

    if (elements.taskModal) {
        const responseEnabled = elements.taskModal.querySelector('#responseEnabled');
        if (responseEnabled) {
            responseEnabled.addEventListener('change', () => {
                const responseOptions = elements.taskModal.querySelector('#responseOptions');
                if (responseOptions) {
    ...
    window.onclick = (event) => {
        ...
        if (event.target === elements.loginModal) elements.loginModal.style.display = 'none';
        if (event.target === elements.triggerModal) elements.triggerModal.style.display = 'none';
        if (event.target === elements.backupSection?.restoreBackupModal) {
            elements.backupSection.restoreBackupModal.style.display = 'none';
        }
    };
    
    // Backup section event listeners
    if (elements.backupSection) {
        // Manual backup button
        if (elements.backupSection.manualBackupBtn) {
            elements.backupSection.manualBackupBtn.addEventListener('click', () => {
                state.socket.emit('trigger_backup');
                showToast('💾 Backup en progreso...', 'info');
            });
        }
        
        // Restore backup modal button
        if (elements.backupSection.restoreBackupBtn) {
            elements.backupSection.restoreBackupBtn.addEventListener('click', () => {
        ...
        // Close restore modal
        document.body.addEventListener('click', (e) => {
            if (e.target.dataset.action === 'close-restore-modal') {
                if (elements.backupSection?.restoreBackupModal) {
                    elements.backupSection.restoreBackupModal.style.display = 'none';
                }
            }
        ...
        document.body.addEventListener('click', async (e) => {
            if (e.target.dataset.action === 'confirm-restore-backup') {
                const filename = elements.backupSection?.restoreBackupSelect?.value;
                if (!filename) {
                    showToast('Selecciona un backup', 'warning');
                    return;
                }
                
                const confirmed = await ui.showConfirmDialog({
                    title: 'Restaurar Backup',
                    message: '¿Estás seguro? Esto reemplazará la base de datos actual.',
                    type: 'danger',
                    confirmText: 'Restaurar'
                });
                
                if (confirmed) {
                    state.socket.emit('restore_backup', { filename: filename });
                    if (elements.backupSection?.restoreBackupModal) {
                        elements.backupSection.restoreBackupModal.style.display = 'none';
                    }
                }
        ...
        document.body.addEventListener('click', async (e) => {
            if (e.target.dataset.action === 'restore-backup-item') {
                const filename = e.target.dataset.filename;
                
                const confirmed = await ui.showConfirmDialog({
                    title: 'Restaurar Backup',
                    message: `¿Restaurar "${filename}"? Esto reemplazará la base de datos actual.`,
                    type: 'danger',
                    confirmText: 'Restaurar'
                });
        ...
        document.body.addEventListener('click', async (e) => {
            if (e.target.dataset.action === 'delete-backup-item') {
                const filename = e.target.dataset.filename;
                ...
                if (confirmed) {
                    state.socket.emit('delete_backup', { filename: filename });
                }
            }
        });
        
        // Update save settings to include backup options
        const originalSaveSettings = document.body.querySelector('[data-action="save-settings"]');
        if (originalSaveSettings) {
            originalSaveSettings.removeEventListener('click', handleSaveSettings);
            originalSaveSettings.addEventListener('click', handleSaveSettings);
        }
    }
}
