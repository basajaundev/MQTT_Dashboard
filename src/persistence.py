...
def get_local_timezone_offset():
    """Retorna el offset de zona horaria local en formato +HH:00 o -HH:00."""
...
def format_timestamp_with_tz(dt):
    """Formatea un datetime con offset de zona horaria local."""
    ...
    try:
        ...
        if total_points > MAX_POINTS:
            ...
            for i in range(MAX_POINTS):
                index = int(i * step)
                row = results[index]
                data.append({'id': row.id, 'device_id': row.device_id, 'location': row.location, 'timestamp': format_timestamp_with_tz(row.timestamp), 'temp_c': row.temp_c, 'temp_h': row.temp_h, 'temp_st': row.temp_st})
            logger.info(f"Downsampling aplicado: {total_points} -> {len(data)} puntos.")
            return data
        else:
            return [{'id': row.id, 'device_id': row.device_id, 'location': row.location, 'timestamp': format_timestamp_with_tz(row.timestamp), 'temp_c': row.temp_c, 'temp_h': row.temp_h, 'temp_st': row.temp_st} for row in results]
    except Exception as e:
